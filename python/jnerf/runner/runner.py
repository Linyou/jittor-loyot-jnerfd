...
class Runner():
    def __init__(self):
        ...
        if self.cfg.ckpt_path and self.cfg.ckpt_path is not None:
            ...
            self.save_path = os.path.join(self.cfg.log_dir, self.exp_name) + f'-{otherStyleTime}'
            if not os.path.exists(self.save_path):
                os.makedirs(self.save_path)
            shutil.copy(self.cfg.filename, self.save_path)
    ...
    def train(self):
        ...
        for i in tqdm(range(self.start, self.tot_train_steps)):
            ...
            if i>0 and i%self.val_freq==0:
                ...
                if best_psnr <= psnr:
                    best_psnr = psnr
                    self.save_ckpt(os.path.join(self.save_path, "best_params.pkl"))
